...
class ImageNavigator {
  ...
  // Invalidate preload cache for specific file paths
  invalidatePreloadCacheForPaths(paths) {
    for (const filePath of paths) {
      const data = this.preloadCache.get(filePath);
      if (data) {
        if (data.img) {
          data.img.src = ""; // Release memory
        }
        this.preloadCache.delete(filePath);
      }
    }
  }

  // Invalidate preload cache for files in specific directories
  invalidatePreloadCacheForDirectories(directories) {
    const pathsToRemove = [];
    for (const cachedPath of this.preloadCache.keys()) {
      const dir = path.dirname(cachedPath);
      if (directories.has(dir)) {
        pathsToRemove.push(cachedPath);
      }
    }
    this.invalidatePreloadCacheForPaths(pathsToRemove);
  }
