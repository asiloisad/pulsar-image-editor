...
module.exports = {
  ...
  openFromDataUrl(dataUrl, title = "Untitled Image") {
    ...
    atom.workspace.getActivePane().activateItem(
      atom.workspace.getActivePane().addItem(imageEditor)
    );
    return imageEditor;
  },
  ...
  handleFileSystemChanges(events) {
    // Collect affected directories and deleted/renamed paths from image file events
    const affectedDirs = new Set();
    const deletedOrRenamedPaths = new Set();
    ...
    for (const event of events) {
      ...
      if (imageExtensions.includes(ext)) {
        ...
        if (event.action === "deleted") {
          deletedOrRenamedPaths.add(event.path);
        }
      }
      ...
      if (event.action === "renamed" && event.oldPath) {
        ...
        if (imageExtensions.includes(oldExt)) {
          ...
          deletedOrRenamedPaths.add(event.oldPath); // Old path is now invalid
        }
      }
    }
    ...
    paneItems.forEach((item) => {
      if (item instanceof ImageEditor && item.view && item.view.navigator) {
        ...
        if (cache.directory && affectedDirs.has(cache.directory)) {
          ...
          // Also invalidate preload cache for affected directories
          navigator.invalidatePreloadCacheForDirectories(affectedDirs);
        }

        // Clear any specifically deleted/renamed paths from preload cache
        if (deletedOrRenamedPaths.size > 0) {
          navigator.invalidatePreloadCacheForPaths([...deletedOrRenamedPaths]);
        }
      }
    });
  },
