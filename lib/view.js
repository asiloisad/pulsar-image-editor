...
module.exports = class ImageEditorView {
  constructor(editor) {
    ...
    this.enablePreloading = atom.config.get("image-editor.enablePreloading") !== false;
  ...
  _setupDisposables() {
    ...
    this.disposables.add(
      atom.commands.add(this.element, {
        ...
        "claude-chat:attach": () => this.attachToClaudeChat(),
    ...
    this.disposables.add(
      ...
      atom.config.onDidChange("image-editor.enablePreloading", ({ newValue }) => {
        this.enablePreloading = newValue !== false;
      }),
  ...
  _setupTooltips() {
    const tooltips = [
      ...
      [this.refs.zoomToFitButton, { title: "Scale image to fit, but not more than 100%", keyBindingCommand: "image-editor:zoom-to-fit" }],
  ...
  zoomToFit() {
    this._zoomToFit(1, true, this.refs.zoomToFitButton);
  }
  ...
  async nextImage() {
    ...
    if (await this.navigator.isAtEnd(this.editor.getPath())) {
  ...
  async previousImage() {
    ...
    if (await this.navigator.isAtStart(this.editor.getPath())) {
  ...
  attachToClaudeChat() {
    ...
    if (this.selectionVisible) {
      const x1 = Math.round(Math.min(this.selectionStartImg.x, this.selectionEndImg.x));
      const y1 = Math.round(Math.min(this.selectionStartImg.y, this.selectionEndImg.y));
      const x2 = Math.round(Math.max(this.selectionStartImg.x, this.selectionEndImg.x));
      const y2 = Math.round(Math.max(this.selectionStartImg.y, this.selectionEndImg.y));
      if (x2 > x1 && y2 > y1) {
