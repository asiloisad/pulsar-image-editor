...
module.exports = class ImageEditorView {
  ...
  async loadImageWithoutFlicker(imagePath) {
    if (!imagePath) return;
  ...
  applyGrayscale() {
    ...
    this.updateImageFromCanvasWithoutHistory(
      canvas,
      `Grayscale applied to ${area.hasSelection ? "selection" : "image"}`,
      () => this.returnCanvasToPool(canvas),
    );
  }
  ...
  showRotateAngleDialog() {
    ...
    const previewSourceCanvas = this.createRotatePreviewCanvas(this.refs.image, 1280);
    ...
    previewContainer.className = "dialog-rotate-preview";

    const previewCanvas = document.createElement("canvas");
    previewContainer.appendChild(previewCanvas);
    dialogElement.appendChild(previewContainer);

    const previewInfo = dialogs.createSelectionInfo("");
    dialogElement.appendChild(previewInfo);

    let previewTimeout = null;
    const schedulePreview = () => {
      if (previewTimeout) clearTimeout(previewTimeout);
      previewTimeout = setTimeout(() => {
        this.renderRotatePreview(
          previewSourceCanvas,
          previewCanvas,
          previewInfo,
    ...
    angleSlider.addEventListener("input", () => {
      valueLabel.textContent = `${angleSlider.value} deg`;
      schedulePreview();
    });
    trimCheckbox.addEventListener("change", schedulePreview);

    const resizePreview = () => {
      const width = Math.max(160, Math.round(previewContainer.clientWidth - 16));
      const height = Math.max(120, Math.round(previewContainer.clientHeight - 16));
  ...
  createRotatePreviewCanvas(imageElement, maxSize) {
  ...
  renderRotatePreview(
    ...
    previewInfo.textContent = `Output: ${outputSize.width} x ${outputSize.height} px`;
  }
