...
module.exports = class ImageEditorView {
  ...
  // Spinner helper methods for loading states
  showSpinner() {
  ...
  loadImageWithoutFlicker(imagePath) {
    ...
    if (
      ...
      // Need to load it first - show spinner
      this.showSpinner();
  ...
  cropToSelection() {
    ...
    // Show spinner during crop operation
    this.showSpinner();
  ...
  // Unified method for blur and sharpen operations
  applyImageFilter(filterType, strength) {
    ...
    const isLargeArea = width * height > 2000000; // > 2 megapixels
    if (isLargeArea) {
    ...
    setTimeout(() => {
      ...
      if (filterType === "blur") {
        const radius = strength * 2; // Scale blur level
        this.fastGaussianBlur(imageData, width, height, radius);
      } else if (filterType === "sharpen") {
        this.applySharpenKernel(imageData, width, height, strength);
      }

      // Put the processed data back to main canvas
      ...
      let message;
      if (filterType === "blur") {
        message = `Blur level ${strength} applied to ${areaText}`;
      } else {
        const strengthName =
          strength <= 0.5 ? "light" : strength <= 1.0 ? "medium" : "strong";
        message = `Sharpen (${strengthName}) applied to ${areaText}`;
      }
  ...
  // Sharpen kernel convolution (extracted from sharpenImage)
  applySharpenKernel(imageData, width, height, strength) {
  ...
  editInPaint() {
    ...
    const extension = path.extname(itemPath).toLowerCase().substring(1);

    if (!supportedExtensions.includes(extension)) {
      atom.notifications.addWarning("Unsupported format for Paint", {
        description: `Paint supports: ${supportedExtensions.join(", ")}`,
      });
      return;
    }

    cp.execFile("mspaint", [itemPath], (error) => {
      if (error) {
        atom.notifications.addError("Failed to open Paint", {
          description: error.message,
        });
      }
    });
  ...
  // Rename/move the current image file using tree-view's MoveDialog
  moveFile() {
    const currentPath = this.editor.getPath();
    if (!currentPath) {
      atom.notifications.addError("No file path available");
    ...
    try {
      ...
      atom.notifications.addError("Failed to open rename dialog", {
        description: error.message,
      });
    }
  }

  // Duplicate the current image file using tree-view's CopyDialog
  duplicateFile() {
    const currentPath = this.editor.getPath();
    if (!currentPath) {
      atom.notifications.addError("No file path available");
    ...
    try {
      ...
      const dialog = new CopyDialog(currentPath, {
        onCopy: ({ initialPath, newPath }) => {
          // File copied successfully - CopyDialog opens the new file automatically
        },
      });
      dialog.attach();
    } catch (error) {
      atom.notifications.addError("Failed to open duplicate dialog", {
        description: error.message,
      });
    }
  }
  ...
  showResizeDialog() {
    ...
    titleElement.textContent = "Resize Image";
    ...
    infoElement.textContent = `Current size: ${currentWidth} × ${currentHeight} px`;
    dialogElement.appendChild(infoElement);

    // Width input
    const widthContainer = document.createElement("div");
    widthContainer.className = "dialog-slider-container";
    const widthLabel = document.createElement("label");
    widthLabel.className = "dialog-label";
    widthLabel.textContent = "Width (px)";
    widthContainer.appendChild(widthLabel);

    const widthInput = document.createElement("input");
    widthInput.type = "number";
    widthInput.className = "input-text native-key-bindings";
    ...
    heightContainer.className = "dialog-slider-container";
    const heightLabel = document.createElement("label");
    heightLabel.className = "dialog-label";
    heightLabel.textContent = "Height (px)";
    heightContainer.appendChild(heightLabel);

    const heightInput = document.createElement("input");
    heightInput.type = "number";
    heightInput.className = "input-text native-key-bindings";
    ...
    lockContainer.className = "dialog-slider-container";
    lockContainer.style.flexDirection = "row";
    ...
    lockLabel.htmlFor = "lock-aspect-ratio";
    ...
    // Percentage resize shortcuts
    const percentContainer = document.createElement("div");
    percentContainer.className = "dialog-slider-container";
    percentContainer.style.display = "flex";
    percentContainer.style.gap = "5px";
    percentContainer.style.flexWrap = "wrap";

    [25, 50, 75, 100, 150, 200].forEach((percent) => {
      const btn = document.createElement("button");
      btn.className = "btn btn-sm";
      btn.textContent = `${percent}%`;
      btn.addEventListener("click", () => {
        const newWidth = Math.round(currentWidth * (percent / 100));
        const newHeight = Math.round(currentHeight * (percent / 100));
        widthInput.value = newWidth;
        heightInput.value = newHeight;
      });
      percentContainer.appendChild(btn);
    });
    dialogElement.appendChild(percentContainer);

    // Link width/height inputs when aspect ratio is locked
    ...
    applyButton.textContent = "Resize";
  ...
  resizeImage(newWidth, newHeight) {
    ...
    ctx.drawImage(this.refs.image, 0, 0, newWidth, newHeight);
  ...
  showFreeRotateDialog() {
    ...
    // Store original image data for preview/cancel
    ...
    const titleElement = document.createElement("h3");
    titleElement.className = "dialog-title";
    titleElement.textContent = "Free Rotate";
    dialogElement.appendChild(titleElement);
    ...
    sliderLabel.className = "dialog-label";
    sliderLabel.textContent = "Angle (degrees)";
    ...
    // Preview function - rotates without saving to history
    const applyPreview = () => {
      if (!autoPreviewCheckbox.checked) return;

      const angle = parseFloat(angleSlider.value);
      const expand = expandCheckbox.checked;
      ...
      // Need to load original image for preview
      const tempImg = new Image();
      tempImg.onload = () => {
    ...
    const schedulePreview = () => {
      ...
      previewTimeout = setTimeout(applyPreview, 150);
    };
    ...
    controlsContainer.appendChild(angleSlider);
    ...
    dialogElement.appendChild(quickContainer);

    // Expand canvas checkbox
    const expandContainer = document.createElement("div");
    expandContainer.className = "dialog-slider-container";
    expandContainer.style.flexDirection = "row";
    expandContainer.style.alignItems = "center";
    expandContainer.style.gap = "8px";

    const expandCheckbox = document.createElement("input");
    expandCheckbox.type = "checkbox";
    expandCheckbox.id = "expand-canvas";
    expandCheckbox.checked = true;

    const expandLabel = document.createElement("label");
    expandLabel.htmlFor = "expand-canvas";
    expandLabel.textContent = "Expand canvas to fit rotated image";
    expandLabel.style.margin = "0";

    expandCheckbox.addEventListener("change", schedulePreview);

    expandContainer.appendChild(expandCheckbox);
    expandContainer.appendChild(expandLabel);
    dialogElement.appendChild(expandContainer);

    // Auto preview checkbox
    const autoPreviewContainer = document.createElement("div");
    autoPreviewContainer.className = "dialog-slider-container";
    autoPreviewContainer.style.flexDirection = "row";
    autoPreviewContainer.style.alignItems = "center";
    autoPreviewContainer.style.gap = "8px";

    const autoPreviewCheckbox = document.createElement("input");
    autoPreviewCheckbox.type = "checkbox";
    autoPreviewCheckbox.id = "auto-preview";
    autoPreviewCheckbox.checked = false;

    const autoPreviewLabel = document.createElement("label");
    autoPreviewLabel.htmlFor = "auto-preview";
    autoPreviewLabel.textContent = "Auto preview";
    autoPreviewLabel.style.margin = "0";

    autoPreviewCheckbox.addEventListener("change", () => {
      if (autoPreviewCheckbox.checked) {
        schedulePreview();
      } else {
        // Restore original when turning off preview
        if (previewTimeout) {
          clearTimeout(previewTimeout);
        }
    ...
    autoPreviewContainer.appendChild(autoPreviewCheckbox);
    autoPreviewContainer.appendChild(autoPreviewLabel);
    ...
    const applyButton = document.createElement("button");
    applyButton.className = "btn btn-primary";
    applyButton.textContent = "Rotate";
    ...
    applyButton.addEventListener("click", () => {
      ...
      // Restore original first, then apply with history
      restoreOriginal();
  ...
  freeRotate(degrees, expandCanvas = true) {
    ...
    canvas.toBlob((blob) => {
      ...
      this.refs.image.onload = () => {
        ...
        if (atom.config.get("image-editor.showSuccessMessages.transform")) {
          atom.notifications.addSuccess("Image rotated", {
            description: `Rotated ${degrees}°. Use "Save" to save changes.`,
          });
        }
      };
      this.refs.image.onerror = () => {
  ...
  updateImageFromCanvasWithoutHistory(
    ...
    // Show spinner during canvas to image conversion
    this.showSpinner();
  ...
  loadFromHistory() {
    ...
    // Show spinner during history load
    this.showSpinner();
