...
module.exports = class ImageEditorView {
  ...
  _setupDisposables() {
    ...
    this.disposables.add(
      atom.commands.add(this.element, {
        ...
        "image-editor:toggle-pending": () => this.togglePending(),
      })
    );
  ...
  _svgBlobUrl(imageUrl, filePath) {
    ...
    try {
      ...
      const cleaned = attrs.replace(/\bwidth=["'][^"']*["']/g, "").replace(/\bheight=["'][^"']*["']/g, "");
      const fixed = svgContent.replace(/<svg([^>]*)>/i, `<svg${cleaned} width="${w}" height="${h}">`);
      return URL.createObjectURL(new Blob([fixed], { type: "image/svg+xml" }));
    } catch {}
    return imageUrl;
  }
  ...
  applyImageFilter(filterType, strength) {
    ...
    setTimeout(() => {
      ...
      this.updateImageFromCanvasWithoutHistory(canvas, message, () => this.returnCanvasToPool(canvas));
    }, 10);
  }
  ...
  applyGrayscale() {
    ...
    this.updateImageFromCanvasWithoutHistory(canvas, `Grayscale applied to ${area.hasSelection ? "selection" : "image"}`, () => this.returnCanvasToPool(canvas));
  }
  ...
  invertColors() {
    ...
    this.updateImageFromCanvasWithoutHistory(canvas, `Colors inverted on ${area.hasSelection ? "selection" : "image"}`, () => this.returnCanvasToPool(canvas));
  }
  ...
  applySepia() {
    ...
    this.updateImageFromCanvasWithoutHistory(canvas, `Sepia tone applied to ${area.hasSelection ? "selection" : "image"}`, () => this.returnCanvasToPool(canvas));
  }
  ...
  applyBrightnessContrast(brightness, contrast) {
    ...
    this.updateImageFromCanvasWithoutHistory(canvas, `Brightness & contrast adjusted on ${area.hasSelection ? "selection" : "image"}`, () => this.returnCanvasToPool(canvas));
  }
  ...
  applySaturation(saturation) {
    ...
    this.updateImageFromCanvasWithoutHistory(canvas, `Saturation adjusted on ${area.hasSelection ? "selection" : "image"}`, () => this.returnCanvasToPool(canvas));
  }
  ...
  applyHueShift(hueShift) {
    ...
    this.updateImageFromCanvasWithoutHistory(canvas, `Hue shifted on ${area.hasSelection ? "selection" : "image"}`, () => this.returnCanvasToPool(canvas));
  }
  ...
  applyPosterize(levels) {
    ...
    this.updateImageFromCanvasWithoutHistory(canvas, `Posterized to ${levels} levels on ${area.hasSelection ? "selection" : "image"}`, () => this.returnCanvasToPool(canvas));
  }
  ...
  autoAdjustColors() {
    ...
    this.updateImageFromCanvasWithoutHistory(canvas, `Auto adjusted colors on ${area.hasSelection ? "selection" : "image"}`, () => this.returnCanvasToPool(canvas));
  }
  ...
  async save() {
    ...
    if (this.readOnly) { atom.notifications.addWarning("This image is read-only"); return; }
  ...
  togglePending() {
    ...
    if (!pane) return;
    const isPending = pane.getPendingItem() === this.editor;
    if (isPending) {
      pane.clearPendingItem();
    } else {
      pane.setPendingItem(this.editor);
    }
    // Tabs package has no event for an item becoming pending,
    // so update the tab CSS class directly
    const tab = pane.getElement().closest("atom-pane")
      ?.querySelector(`.tab-bar`)?.querySelectorAll(".tab");
    if (tab) {
      for (const t of tab) {
        if (t.item === this.editor) {
          t.classList.toggle("pending-tab", !isPending);
          t.querySelector(".title")?.classList.toggle("temp", !isPending);
          break;
        }
      }
    }
