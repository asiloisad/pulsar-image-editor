  ...
  class ImageEditorView {
    ...
    firstImage() {
      ...
      try {
        ...
        if (files.length > 0) {
          const firstPath = path.join(directory, files[0])
          this.editor.load(firstPath)
        }
      } catch (e) {
        console.error('Error finding first image:', e)
      }
    }
    ...
    lastImage() {
      ...
      try {
        ...
        if (files.length > 0) {
          const lastPath = path.join(directory, files[files.length - 1])
          this.editor.load(lastPath)
        }
      } catch (e) {
        console.error('Error finding last image:', e)
      }
    }
