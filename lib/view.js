  ...
  class ImageEditorView {
    ...
    preloadImage(imagePath) {
      try {
        ...
        img.src = encodedPath
        this.preloadCache.set(imagePath, img)
      } catch (e) {
    ...
    centerImage() {
      ...
      if (this.originX === 0 && this.originY === 0) {
        this.originX = containerWidth / 2
        this.originY = containerHeight / 2
      }

      // Position image so its center aligns with the origin
      const imageWidth = this.refs.image.naturalWidth * this.zoom
      const imageHeight = this.refs.image.naturalHeight * this.zoom

      this.translateX = this.originX - imageWidth / 2
      this.translateY = this.originY - imageHeight / 2
    ...
    autoSelect() {
      ...
      atom.notifications.addSuccess('Auto-selection complete', {
    ...
    updateImageFromCanvasWithoutHistory(canvas, successMessage, configKey = 'adjustment', saveToHistoryAfter = false) {
      canvas.toBlob((blob) => {
        ...
        this.refs.image.onload = () => {
          ...
          this.updateTransform()
          this.updateTransform()
          // this.refs.selectionBox.style.display = 'none' // Keep selection visible for filters/adjustments
          this.emitter.emit('did-update')
