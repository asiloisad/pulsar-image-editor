...
module.exports =
  class ImageEditorView {
    constructor(editor) {
      ...
      this.history = []
      this.historyIndex = -1
      this.maxHistorySize = atom.config.get('image-editor.maxHistorySize') || 20
      ...
      this.disposables.add(atom.tooltips.add(this.refs.firstImageButton, { title: 'Navigate to the first image in the current directory', keyBindingCommand: 'image-editor:first-image' }))
      this.disposables.add(atom.tooltips.add(this.refs.prevImageButton, { title: 'Navigate to the previous image', keyBindingCommand: 'image-editor:previous-image' }))
      this.disposables.add(atom.tooltips.add(this.refs.undoButton, { title: 'Undo last change', keyBindingCommand: 'image-editor:undo' }))
      this.disposables.add(atom.tooltips.add(this.refs.redoButton, { title: 'Redo last undone change', keyBindingCommand: 'image-editor:redo' }))
      this.disposables.add(atom.tooltips.add(this.refs.nextImageButton, { title: 'Navigate to the next image', keyBindingCommand: 'image-editor:next-image' }))
      this.disposables.add(atom.tooltips.add(this.refs.lastImageButton, { title: 'Navigate to the last image in the current directory', keyBindingCommand: 'image-editor:last-image' }))

      // Add tooltips for zoom buttons
      this.disposables.add(atom.tooltips.add(this.refs.zoomOutButton, { title: 'Decrease zoom level', keyBindingCommand: 'image-editor:zoom-out' }))
      this.disposables.add(atom.tooltips.add(this.refs.resetZoomButton, { title: 'Reset zoom to default', keyBindingCommand: 'image-editor:reset-zoom' }))
      this.disposables.add(atom.tooltips.add(this.refs.zoomInButton, { title: 'Increase zoom level', keyBindingCommand: 'image-editor:zoom-in' }))
      this.disposables.add(atom.tooltips.add(this.refs.centerButton, { title: 'Center the image within the pane', keyBindingCommand: 'image-editor:center' }))
      this.disposables.add(atom.tooltips.add(this.refs.zoomToFitButton, { title: 'Scale image to fit the window', keyBindingCommand: 'image-editor:zoom-to-fit' }))
      this.disposables.add(atom.tooltips.add(this.refs.zoomTo100Button, { title: 'Scale image to fit, but not more than 100%', keyBindingCommand: 'image-editor:zoom-to-100' }))
      ...
      this.disposables.add(new Disposable(() => { this.refs.undoButton.removeEventListener('click', undoClickHandler) }))

      const redoClickHandler = () => {
        this.redo()
      }
      this.refs.redoButton.addEventListener('click', redoClickHandler)
      this.disposables.add(new Disposable(() => { this.refs.redoButton.removeEventListener('click', redoClickHandler) }))
      ...
      this.refs.firstImageButton.addEventListener('click', firstImageClickHandler)
      this.disposables.add(new Disposable(() => { this.refs.firstImageButton.removeEventListener('click', firstImageClickHandler) }))
      ...
      this.disposables.add(new Disposable(() => { this.refs.lastImageButton.removeEventListener('click', lastImageClickHandler) }))
      ...
      this.mouseMoveHandler = (event) => {
        ...
        if (this.loaded && this.refs.image) {
          ...
          if (imgX >= 0 && imgX < this.refs.image.naturalWidth && imgY >= 0 && imgY < this.refs.image.naturalHeight) {
    ...
    async updateImageURI() {
      ...
      if (isLargeImage) {
        ...
        // console.log(`Loading large image: ${bytes(this.imageSize)}`)
      }
      ...
      try {
        ...
        if (!currentLoad.cancelled) {
          ...
          // Only show notification for non-decode errors or if it's the current load
          if (!error.message.includes('decode') || error.name !== 'AbortError') {
          ...
          try {
            ...
            if (isLargeImage && this.refs.image.decode) {
              try {
                ...
                throw decodeError // Unexpected error, rethrow
              }
            }
            ...
            if (isLargeImage) {
              ...
              // console.log(`Image loaded in ${loadTime}ms (${this.originalWidth}x${this.originalHeight})`)
            }
    ...
    getPooledCanvas(width, height) {
      ...
      const canvasIndex = this.canvasPool.findIndex(c =>
        c.width >= width && c.height >= height &&
        c.width < width * 1.5 && c.height < height * 1.5 // Not too oversized
      )
    ...
    preloadImageWithZoom(imagePath) {
      try {
        const img = new Image()
        const encodedPath = `file://${encodeURI(imagePath.replace(/\\/g, '/')).replace(/#/g, '%23').replace(/\?/g, '%3F')}`
    ...
    getFileList() {
      ...
      if (this.fileListCache.directory === directory && this.fileListCache.files.length > 0) {
        ...
        this.fileListCache.currentIndex = this.fileListCache.files.findIndex(f =>
          path.normalize(f).toLowerCase() === path.normalize(currentPath).toLowerCase()
        )
        ...
        try {
          ...
          if (dirEntry && dirEntry.children) {
            ...
            files = Array.from(dirEntry.children)
              ...
              .sort((a, b) => path.basename(a).localeCompare(path.basename(b), undefined, {
                numeric: true,
                sensitivity: 'base'
              }))
          }
        } catch (e) {
          // Tree-view method failed, fall back to filesystem
          // console.log('Tree-view not available, using filesystem')
        }
      }
      ...
      if (files.length === 0) {
        try {
          files = fs.readdirSync(directory)
            .filter(file => this.fileListCache.extensions.includes(path.extname(file).toLowerCase()))
          ...
          return { directory: null, files: [], currentIndex: -1, extensions: this.fileListCache.extensions }
        }
      }
      ...
      this.fileListCache.currentIndex = files.findIndex(f =>
        path.normalize(f).toLowerCase() === path.normalize(currentPath).toLowerCase()
      )
    ...
    loadImageWithoutFlicker(imagePath) {
      ...
      if (preloadedData && preloadedData.img && preloadedData.img.complete && preloadedData.img.naturalWidth > 0) {
        ...
        const encodedPath = `file://${encodeURI(imagePath.replace(/\\/g, '/')).replace(/#/g, '%23').replace(/\?/g, '%3F')}?time=${Date.now()}`
    ...
    cropToSelection() {
      ...
      canvas.toBlob((blob) => {
        ...
        const cropCenterX = this.translateX + (imgLeft + imgWidth / 2) * this.zoom
        const cropCenterY = this.translateY + (imgTop + imgHeight / 2) * this.zoom
    ...
    autoSelect(borderPercent = 0) {
      ...
      if (left >= right || top >= bottom) {
        atom.notifications.addWarning('No content detected', {
          description: 'Could not detect content boundaries. Try adjusting the tolerance in settings.'
        })
        return
      }
      ...
      if (borderPercent > 0) {
        // Calculate border based on smaller content dimension (not image dimension)
        const contentWidth = right - left + 1
        const contentHeight = bottom - top + 1
        const smallerContentDimension = Math.min(contentWidth, contentHeight)
        const border = Math.round(smallerContentDimension * borderPercent / 100)
    ...
    showPropertiesDialog() {
      ...
      try {
        ...
        atom.notifications.addError('Cannot read file stats', { description: e.message })
        return
      }
      ...
      addRow('Print size (96 DPI)', `${printWidthInches} × ${printHeightInches} inches (${printWidthCm} × ${printHeightCm} cm)`)
      ...
      if (fileList.currentIndex >= 0 && fileList.files.length > 0) {
        addRow('Position in folder', `${fileList.currentIndex + 1} / ${fileList.files.length}`)
      }

      // History info
      if (this.history.length > 0) {
        addRow('History', `${this.historyIndex + 1} / ${this.history.length} states`)
      }
      ...
      analyzeButton.addEventListener('click', () => {
        ...
        setTimeout(() => {
          try {
            ...
            const estimatedTotal = sampleRate > 1 ? `~${(uniqueColors * sampleRate).toLocaleString()}` : uniqueColors.toLocaleString()

            colorInfoDiv.innerHTML = `
              <table class="properties-table">
                <tr><td class="property-label">Unique colors:</td><td class="property-value">${estimatedTotal}</td></tr>
                <tr><td class="property-label">Color depth:</td><td class="property-value">24-bit RGB${hasAlpha ? ' + Alpha' : ''}</td></tr>
                <tr><td class="property-label">Transparency:</td><td class="property-value">${hasAlpha ? 'Yes' : 'No'}</td></tr>
              </table>
            `
    ...
    showAdjustmentDialog(title, sliders, applyCallback) {
      ...
      cancelButton.addEventListener('click', () => {
        ...
        this.emitter.off('selection-visibility-changed', selectionVisibilityHandler)
        document.body.removeChild(backdrop)
      })
    ...
    applyBrightnessContrast(brightness, contrast) {
      ...
      const areaText = area.hasSelection ? 'selection' : 'image'
      this.updateImageFromCanvasWithoutHistory(canvas, `Brightness & contrast adjusted on ${areaText}`, 'adjustment', false)
    }
    ...
    applySaturation(saturation) {
      ...
      const areaText = area.hasSelection ? 'selection' : 'image'
      this.updateImageFromCanvasWithoutHistory(canvas, `Saturation adjusted on ${areaText}`, 'adjustment', false)
    }
    ...
    autoAdjustColors() {
      ...
      const areaText = area.hasSelection ? 'selection' : 'image'
      this.updateImageFromCanvasWithoutHistory(canvas, `Auto adjusted colors on ${areaText}`, 'adjustment', false)
    }
    ...
    applyHueShift(hueShift) {
      ...
      const areaText = area.hasSelection ? 'selection' : 'image'
      this.updateImageFromCanvasWithoutHistory(canvas, `Hue shifted on ${areaText}`, 'adjustment', false)
    }
    ...
    applyPosterize(levels) {
      ...
      const areaText = area.hasSelection ? 'selection' : 'image'
      this.updateImageFromCanvasWithoutHistory(canvas, `Posterized to ${levels} levels on ${areaText}`, 'adjustment', false)
    }
