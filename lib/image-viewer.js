...
const ImageViewerView = require('./image-viewer-view')
...
module.exports = class ImageViewer {
  static deserialize ({filePath}) {
    ...
    if (fileStats?.isFile()) {
      return new ImageViewer(filePath)
    } else {
  ...
  copy () {
    return new ImageViewer(this.getPath())
  }
  ...
  get view () {
    if (!this.editorView) {
      try {
        this.editorView = new ImageViewerView(this)
      } catch (e) {
        console.warn(`Could not create ImageViewerView. This can be intentional in the event of an image file being deleted by an external program.`)
        return
      }
    }
  ...
  // Compares two {ImageViewer}s to determine equality.
  //
  // Equality is based on the condition that the two URIs are the same.
  //
  // Returns a {Boolean}.
  isEqual (other) {
    return other instanceof ImageViewer && (this.getURI() === other.getURI())
  }
